@for ($k = 0; $k < $lastBeforeYearCount; $k++)
    ...
    @if (isset($sourceOfIncomeList[$sourceOfIncomeLastBeforeYear]) || $lastBeforeYearCount == 1)
        <!-- Last Year -->
        <div
            class="light-gray-div mt-2 spouse_last_before_year_row spouse-last-before-year-row-{{ Helper::validate_key_value(2, $taxYears) }}">
            <div class="light-gray-box-form-area">
                ...
                <button type="button" class="delete-div delete-icon" title="Delete"
                    onclick="deleteIncomeRow(this , 'spouse_last_before_year_row');">
                        ...
                        <div class="label-div">
                            ...
                            <div class="amount-last-before-year form-group">
                                ...
                                <select class="form-control required convert-text-for-other"
                                    name="other_income_received_spouse_lastbefore_year[{{ $k }}]">
                                    {!! Helper::getSourceOfIncomeSelection($sourceOfIncomeLastBeforeYear) !!}
                                </select>
                                @php $last_before_year_income_text = Helper::validate_key_loop_value('other_income_received_spouse_lastbefore_year_text', $finacial_affairs, $k); @endphp
                                <div class="other_income {{ $sourceOfIncomeLastBeforeYear != -1 ? 'hide-data' : '' }}">
                                    <input type="text" class="input_capitalize form-control required"
                                        name="other_income_received_spouse_lastbefore_year_text[{{ $k }}]"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group total_amount_this_year_income">
                                    ...
                                    <input type="number" class="form-control price-field mw-unset required"
                                        name="other_amount_spouse_lastbefore_year_income[{{ $k }}]"
                                        value="{{ (float) Helper::validate_key_loop_value('other_amount_spouse_lastbefore_year_income', $finacial_affairs, $k) }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="add-more-div-bottom">
    <button type="button" id="add-more-residence-form" class="btn-new-ui-default py-1 px-2 this_year_second_add_more "
        onclick="addMoreIncomeRow('spouse_last_before_year_row', 'other_income_received_spouse_lastbefore_year')">
