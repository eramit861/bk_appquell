@for ($i = 0; $i < $currentYearCount; $i++)
    ...
    @if (isset($sourceOfIncomeList[$sourceOfIncomeThisYear]) || $currentYearCount == 1)
        <!-- Current Year -->
        <div
            class="light-gray-div mt-2 spouse_current_year_row spouse-income-row-{{ Helper::validate_key_value(0, $taxYears) }}">
            <div class="light-gray-box-form-area">
                ...
                <button type="button" class="delete-div delete-icon" title="Delete"
                    onclick="deleteIncomeRow(this , 'spouse_current_year_row');">
                        ...
                        <div class="label-div">
                            <div class="amount-this-year form-group">
                                ...
                                <select class="form-control required convert-text-for-other"
                                    name="other_income_received_spouse_this_year[{{ $i }}]">
                                    {!! Helper::getSourceOfIncomeSelection($sourceOfIncomeThisYear) !!}
                                </select>
                                @php $other_income_text = Helper::validate_key_loop_value('other_income_received_spouse_this_year_text', $finacial_affairs, $i); @endphp
                                <div class="other_income {{ $sourceOfIncomeThisYear != -1 ? 'hide-data' : '' }}">
                                    <input type="text" class="input_capitalize form-control required"
                                        name="other_income_received_spouse_this_year_text[{{ $i }}]"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group total_amount_this_year_income">
                                    ...
                                    <input type="number" class="form-control price-field mw-unset required"
                                        name="other_amount_spouse_this_year_income[{{ $i }}]"
                                        value="{{ (float) Helper::validate_key_loop_value('other_amount_spouse_this_year_income', $finacial_affairs, $i) }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="add-more-div-bottom">
    <button type="button" id="add-more-residence-form" class="btn-new-ui-default py-1 px-2 this_year_second_add_more "
        onclick="addMoreIncomeRow('spouse_current_year_row', 'other_income_received_spouse_this_year')">
