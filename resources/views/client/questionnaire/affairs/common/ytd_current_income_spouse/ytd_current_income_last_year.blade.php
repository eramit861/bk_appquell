@for ($j = 0; $j < $lastYearCount; $j++)
    ...
    @if (isset($sourceOfIncomeList[$sourceOfIncomeLastYear]) || $lastYearCount == 1)
        <!-- Last Year -->
        <div
            class="light-gray-div mt-2 spouse_last_year_row spouse-last-year-row-{{ Helper::validate_key_value(1, $taxYears) }}">
            <div class="light-gray-box-form-area">
                ...
                <button type="button" class="delete-div delete-icon" title="Delete"
                    onclick="deleteIncomeRow(this , 'spouse_last_year_row');">
                        ...
                        <div class="label-div">
                            ...
                            <div class="amount-last-year form-group">
                                ...
                                <select class="form-control required convert-text-for-other"
                                    name="other_income_received_spouse_last_year[{{ $j }}]">
                                    {!! Helper::getSourceOfIncomeSelection($sourceOfIncomeLastYear) !!}
                                </select>
                                @php $last_year_income_text = Helper::validate_key_loop_value('other_income_received_spouse_last_year_text', $finacial_affairs, $j); @endphp
                                <div class="other_income {{ $sourceOfIncomeLastYear != -1 ? 'hide-data' : '' }}">
                                    <input type="text" class="input_capitalize form-control required"
                                        name="other_income_received_spouse_last_year_text[{{ $j }}]"
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group total_amount_this_year_income">
                                    ...
                                    <input type="number" class="form-control price-field mw-unset required"
                                        name="other_amount_spouse_last_year_income[{{ $j }}]"
                                        value="{{ (float) Helper::validate_key_loop_value('other_amount_spouse_last_year_income', $finacial_affairs, $j) }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="add-more-div-bottom">
    <button type="button" id="add-more-residence-form" class="btn-new-ui-default py-1 px-2 this_year_second_add_more "
        onclick="addMoreIncomeRow('spouse_last_year_row', 'other_income_received_spouse_last_year')">
