...
<ul class="nav nav-pills nav-fill w-100 p-0 with-progress" id="pills-tab" role="tablist">
    ...
    @endphp
    <li class="nav-item nav-item-ui-new {{ request()->routeIs('client_expenses') ? 'mobile-mr-0' : 'hide-tab' }} {{ $step1TabClass }}" role="presentation">
        <button class="nav-link tab-ui-new {{ request()->routeIs('client_expenses') ? 'active' : '' }}"
            onclick="redirectToURL('{{ route('client_expenses') }}')"
            id="all-firm-client-tab"
            data-bs-toggle="pill"
            data-bs-target="#all-firm-client"
            type="button" role="tab" aria-controls="all-firm-client" aria-selected="true">
            <span>&#x1F3E0;</span>
            <span>Current Household Expenses</span>
            <div class="progress" role="progressbar" aria-label="Basic example" aria-valuenow="{{ $step1PercentDone }}" aria-valuemin="0" aria-valuemax="{{ $step1PercentTotal }}">
                <div class="progress-bar" style="width:{{ $step1PercentDone ?? 0 }}%"><div class="progress_text_{{ $step1PercentDone ?? 0 }}">{{ $step1PercentDone ?? 0 }}%</div></div>
            </div>
        </button>
    </li>
    @if($showSpouseExpense)
        @php
        $title = 'Spousal Expenses Separate Household';
        ...
        @endphp
        <li class="nav-item nav-item-ui-new {{ request()->routeIs('client_spouse_expenses') ? 'mobile-mr-0' : 'hide-tab' }} {{ $step2TabClass }}" role="presentation">
            <button class="nav-link tab-ui-new {{ request()->routeIs('client_spouse_expenses') ? 'active' : '' }}"
                onclick="redirectToURL('{{ route('client_spouse_expenses') }}')"
                id="all-firm-client-tab"
                data-bs-toggle="pill"
                data-bs-target="#all-firm-client"
                type="button" role="tab" aria-controls="all-firm-client" aria-selected="true">
                ...
                <div class="progress" role="progressbar" aria-label="Basic example" aria-valuenow="{{ $step2PercentDone }}" aria-valuemin="0" aria-valuemax="{{ $step2PercentTotal }}">
                    <div class="progress-bar" style="width:{{ $step2PercentDone ?? 0 }}%"><div class="progress_text_{{ $step2PercentDone ?? 0 }}">{{ $step2PercentDone ?? 0 }}%</div></div>
                </div>
            </button>
        </li>
    @endif
...
<div class="card-body border-top-left-radius-none">
    <div class="tab-content bg-unset p-0 box-shadow-unset" id="pills-tabContent">
        <div class="tab-pane fade show active" id="active" role="tabpanel" aria-labelledby="" tabindex="0">
            ...
            @include("client.questionnaire.expense.steps.step1")
        </div>
    </div>
</div>
...
<script>
    window.__utilityPopupData = {
        ...
        previousData: $('.utility_text_field_d1').val()
    };
</script>
