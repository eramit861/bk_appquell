<div class="col-12">
    <div class="light-gray-div mt-2">
        ...
        <div class="row gx-3">
            ...
            @if (Auth::user()->client_type != Helper::CLIENT_TYPE_INDIVIDUAL_NOT_MARRIED)
                <div class="col-12">
                    <div class="label-div question-area">
                        <label>
                            Do you and your spouse live separately and maintain separate households?
                            <div class="d-inline-block" data-bs-toggle="tooltip" data-bs-placement="top" title=""
                                data-bs-original-title="Indicates whether you and your spouse live apart and maintain independent residences.">
                                <i class="bi bi-question-circle"></i>
                            </div>
                        </label>
                        <div class="custom-radio-group form-group">
                            <input type="radio" name="live_separately" id="live_separately_yes"
                                class="d-none required" value="1"
                                {{ Helper::validate_key_toggle('live_separately', $expenses, 1) }}>
                            <label for="live_separately_yes"
                                class="btn-toggle {{ Helper::validate_key_toggle_active('live_separately', $expenses, 1) }}"
                                onclick="getLiveSeparationData('yes');">Yes</label>

                            <input type="radio" name="live_separately" id="live_separately_no" class="d-none required"
                                value="0" {{ Helper::validate_key_toggle('live_separately', $expenses, 0) }}>
                            <label for="live_separately_no"
                                class="btn-toggle {{ Helper::validate_key_toggle_active('live_separately', $expenses, 0) }}"
                                onclick="getLiveSeparationData('no');">No</label>
                        </div>
                    </div>
                    <div class="form-group {{ Helper::key_hide_show_v('live_separately', $expenses) }}"
                        id="live_separately_data">
                    </div>
                </div>
            @endif
        </div>
