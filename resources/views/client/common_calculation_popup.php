                        ...
                        <tbody>
                            ...
                            foreach ($montYears as $m => $v) {
                                if (!in_array($m, array_column($allReport, 'pay_period_end'))) {
                                    array_push($allReport, ['pay_period_end' => $m, 'gross_pay_amount' => 0, 'total_taxes' => 0, 'total_deductions' => 0]);
                                }
                            }
                            @endphp
                            @if (!empty($allReport[0]))
                                @php
                                // $gross_pay_amount_column =  array_column($allReport,'gross_pay_amount');
                                // $total_taxes_column =  array_column($allReport,'total_taxes');
                                // $total_deductions_column =  array_column($allReport,'total_deductions');
                                // $pay_period_end_column =  array_column($allReport,'pay_period_end');
                                ...
                                @foreach ($montYears as $key => $val)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ Helper::formatPrice($report['gross_pay_amount'] - $report['total_taxes'] - $report['total_deductions']) }}</td>
                                    </tr>
                                @endforeach
                            @endif
                            <tr style=" background-color: rgba(4, 169, 245, 0.05)">
                                <td class="bg-gray-custom text-bold"><strong>6 Month Average</strong></td>
                                <td><strong>{{ $totalGrossPay > 0 ? Helper::formatPrice($totalGrossPay / 6) : '$0' }}</strong></td>
                                <td><strong>{{ $totalTaxes > 0 ? Helper::formatPrice($totalTaxes / 6) : '$0' }}</strong></td>
                                <td><strong>{{ $totalDeduction > 0 ? Helper::formatPrice($totalDeduction / 6) : '$0' }}</strong></td>
                                <td><strong>{{ $totalnetAmount > 0 ? Helper::formatPrice($totalnetAmount / 6) : '$0' }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                    <label class="float_right"><i class="float_right">Net Income is based on formula: Net income = (Gross pay amount - total taxes - total deductions)</i></label>
                </div>
            </div>
        </div>
    </div>
...
<style>
   .card.information-area.calculation td{
