...
class QuestionnairePropertyBusinessAssets extends Model
{
    protected $guarded = [];
    protected $table = 'questionnaire_property_business_assets';
    public $timestamps = false;


    public static function saveStepBusinessAssets($client_id, $request, $save_request_by_attorney = false, $attorney_id = '')
    {
        // get all input data
        $input = $request->except('_token');

        // save business assets
        self::updateOrCreateBusinessAssets($input, $client_id, $save_request_by_attorney, $attorney_id);

        // forget property cache
        CacheProperty::forgetPropertyCache($client_id);
    }

    private static function updateOrCreateBusinessAssets($input, $client_id, $save_request_by_attorney, $attorney_id)
    {
        $abFormLineStart = QuestionnaireUser::get_AB_FORM_LINE_NUMBERS('business_related_assets');
        $abFormLineNo = '';
        if (!empty($input)) {
            ...
            $function = $save_request_by_attorney ? 'questionnairePropertyBusinessAssets' : 'clientsPropertyBusinessAssets';
